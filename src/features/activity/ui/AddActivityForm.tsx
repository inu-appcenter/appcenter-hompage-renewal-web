...
export function AddActivityForm() {
  ...
  // 섹션 이미지 개별 삭제
  const removeSectionImage = (id: number, imageIndex: number) => {
    setForm({
      ...form,
      contents: form.contents.map((s) => {
        if (s.id === id) {
          const newImages = s.imageUrls.filter((_, i) => i !== imageIndex);
          return { ...s, imageUrls: newImages };
        }
        return s;
      })
    });
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
      <h1 className="mb-8 text-2xl font-bold text-slate-900">새 활동 게시글 등록</h1>
                      ...
                      <div className="mb-2 grid grid-cols-3 gap-2">
                        {section.imageUrls.map((url, imgIdx) => (
                          <div key={imgIdx} className="group relative aspect-square overflow-hidden rounded border border-slate-100">
                            ...
                            <button
                              type="button"
                              onClick={() => removeSectionImage(section.sequence, imgIdx)}
